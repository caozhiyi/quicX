...
static std::pair<std::shared_ptr<CryptoT>, std::shared_ptr<CryptoT>> MakeCryptographerPair() {
...
static void BM_Aead_EncryptPacket(benchmark::State& state) {
    ...
    for (auto _ : state) {
        auto out =
            std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(out_cap));

        common::BufferSpan ad(ad_bytes.data(), ad_bytes.data() + ad_bytes.size());
        common::BufferSpan pt(plaintext_bytes.data(), plaintext_bytes.data() + plaintext_bytes.size());
...
static void BM_Aead_DecryptPacket(benchmark::State& state) {
    ...
    std::vector<uint8_t> ct_bytes(ct_span.GetStart(), ct_span.GetStart() + ct_span.GetLength());
...
static void BM_Aead_EncryptHeader(benchmark::State& state) {
    auto crypto = std::make_shared<CryptoT>();
    crypto->InstallInitSecret(kTestDcid, sizeof(kTestDcid), quic::kInitialSaltV1.data(), quic::kInitialSaltV1.size(),
        /*is_server=*/false);
    ...
    for (auto _ : state) {
        ...
        common::BufferSpan sample(sample_bytes.data(), sample_bytes.data() + sample_bytes.size());
...
static void BM_Hkdf_Expand_Sha256_32(benchmark::State& state) {
    ...
    const uint8_t info[] = {'t', 'l', 's', '1', '3', ' ', 'q', 'u', 'i', 'c', ' ', 'k', 'e', 'y'};
    ...
    for (auto _ : state) {
        bool ok = quic::Hkdf::HkdfExpand(
            out.data(), out.size(), kTestTrafficSecret, sizeof(kTestTrafficSecret), info, sizeof(info), md);
