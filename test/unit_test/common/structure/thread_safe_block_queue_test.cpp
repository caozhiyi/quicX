...
TEST(ThreadSafeBlockQueueTest, basic_operations) {
...
TEST(ThreadSafeBlockQueueTest, multiple_elements) {
    ThreadSafeBlockQueue<int> queue;
...
TEST(ThreadSafeBlockQueueTest, try_pop) {
    ThreadSafeBlockQueue<int> queue;
...
TEST(ThreadSafeBlockQueueTest, try_pop_timeout) {
    ThreadSafeBlockQueue<int> queue;
...
TEST(ThreadSafeBlockQueueTest, clear) {
    ThreadSafeBlockQueue<int> queue;
...
// Thread safety tests
TEST(ThreadSafeBlockQueueTest, thread_safety_single_producer_single_consumer) {
    ThreadSafeBlockQueue<int> queue;
...
TEST(ThreadSafeBlockQueueTest, thread_safety_multiple_producers) {
    ThreadSafeBlockQueue<int> queue;
    std::atomic<bool> stop_flag(false);
...
TEST(ThreadSafeBlockQueueTest, thread_safety_multiple_consumers) {
    ThreadSafeBlockQueue<int> queue;
    std::atomic<bool> stop_flag(false);
    std::atomic<int> total_received(0);
...
TEST(ThreadSafeBlockQueueTest, large_number_of_elements) {
    ThreadSafeBlockQueue<int> queue;
...
TEST(ThreadSafeBlockQueueTest, stress_test) {
    ThreadSafeBlockQueue<int> queue;
    std::atomic<bool> stop_flag(false);
...
struct TestData {
    ...
    TestData():
        id(0) {}
    TestData(int i, const std::string& n):
        id(i),
        name(n) {}

    bool operator==(const TestData& other) const { return id == other.id && name == other.name; }
};

TEST(ThreadSafeBlockQueueTest, custom_type) {
