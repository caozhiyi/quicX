...
public:
    MockClientConnection(std::shared_ptr<IQuicRecvStream> stream):
        ...
        unidentified_stream_ = std::make_shared<UnidentifiedStream>(stream,
            ...
            std::bind(&MockClientConnection::OnStreamTypeIdentified, this, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3));
    }
    ...
    void OnStreamTypeIdentified(
        ...
        receiver_stream_ = std::make_shared<ControlReceiverStream>(stream, std::make_shared<QpackEncoder>(),
            std::bind(&MockClientConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2),
...
public:
    MockServerConnection(std::shared_ptr<IQuicSendStream> stream):
        error_code_(0) {
        sender_stream_ = std::make_shared<ControlSenderStream>(
            stream, std::bind(&MockServerConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2));
    }
...
class ControlServerSenderStreamTest: public testing::Test {
protected:
    void SetUp() override {
