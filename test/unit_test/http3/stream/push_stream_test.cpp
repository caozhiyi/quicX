...
public:
    MockClientConnection(const std::shared_ptr<QpackEncoder>& qpack_encoder, std::shared_ptr<IQuicRecvStream> stream):
...
public:
    MockServerConnection(const std::shared_ptr<QpackEncoder>& qpack_encoder, std::shared_ptr<IQuicSendStream> stream):
        error_code_(0) {
...
class PushStreamTest: public testing::Test {
protected:
    void SetUp() override {
