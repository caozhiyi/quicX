...
public:
    MockClientConnection(std::shared_ptr<IQuicSendStream> stream):
        error_code_(0) {
        sender_stream_ = std::make_shared<ControlClientSenderStream>(
            stream, std::bind(&MockClientConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2));
    }
    ~MockClientConnection() {}

    bool SendMaxPushId(uint64_t push_id) { return sender_stream_->SendMaxPushId(push_id); }

    bool SendCancelPush(uint64_t push_id) { return sender_stream_->SendCancelPush(push_id); }

    bool SendGoAway(uint64_t stream_id) { return sender_stream_->SendGoaway(stream_id); }
...
public:
    MockServerConnection(std::shared_ptr<IQuicRecvStream> stream):
        ...
        unidentified_stream_ = std::make_shared<UnidentifiedStream>(stream,
            ...
            std::bind(&MockServerConnection::OnStreamTypeIdentified, this, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3));
    }
    ...
    void OnStreamTypeIdentified(
        ...
        receiver_stream_ = std::make_shared<ControlServerReceiverStream>(stream, std::make_shared<QpackEncoder>(),
            std::bind(&MockServerConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2),
    ...
    void GoAwayHandler(uint64_t id) { goaway_id_ = id; }

    void SettingHandler(const std::unordered_map<uint16_t, uint64_t>& settings) { settings_ = settings; }

    void MaxPushIdHandler(uint64_t push_id) { max_push_id_ = push_id; }

    void CancelHandler(uint64_t id) { cancel_id_ = id; }
...
class ControlClientSenderStreamTest: public testing::Test {
protected:
    void SetUp() override {
...
TEST_F(ControlClientSenderStreamTest, SendSettings) {
    std::unordered_map<uint16_t, uint64_t> settings = {
        {SettingsType::kMaxFieldSectionSize, 100}, {SettingsType::kQpackMaxTableCapacity, 4096}};
