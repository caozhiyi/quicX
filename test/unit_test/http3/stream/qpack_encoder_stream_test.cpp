...
public:
    MockSenderConnection(std::shared_ptr<IQuicSendStream> stream):
        error_code_(0) {
        sender_stream_ = std::make_shared<QpackEncoderSenderStream>(
            stream, std::bind(&MockSenderConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2));
    }
    ~MockSenderConnection() {}

    bool SendSetCapacity(uint64_t capacity) { return sender_stream_->SendSetCapacity(capacity); }
    ...
    bool SendDuplicate(uint64_t index) { return sender_stream_->SendDuplicate(index); }
...
public:
    ...
    void OnStreamTypeIdentified(
        ...
        receiver_stream_ = std::make_shared<QpackEncoderReceiverStream>(stream, qpack_encoder_, blocked_registry_,
            std::bind(&MockReceiverConnection::ErrorHandle, this, std::placeholders::_1, std::placeholders::_2));
    ...
    void NotifyCallback() { notify_count_++; }
...
TEST(QpackEncoderStreamTest, SendInstructionsBlobTest) {
    ...
    std::vector<std::pair<std::string, std::string>> instructions = {
        {"set-capacity", "4096"}};  // Set capacity to some value
...
TEST(QpackEncoderStreamTest, BlockedRegistryNotificationTest) {
    ...
    EXPECT_TRUE(
        receiver->GetBlockedRegistry()->Add(key1, std::bind(&MockReceiverConnection::NotifyCallback, receiver.get())));
    EXPECT_TRUE(
        receiver->GetBlockedRegistry()->Add(key2, std::bind(&MockReceiverConnection::NotifyCallback, receiver.get())));
