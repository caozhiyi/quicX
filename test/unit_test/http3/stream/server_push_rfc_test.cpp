...
protected:
    void SetUp() override {
        ...
        sender_stream_ = std::make_shared<PushSenderStream>(
            qpack_encoder_, mock_send_stream_, [this](uint64_t stream_id, uint32_t error) { error_code_ = error; });

        // Start with UnidentifiedStream to read stream type
        unidentified_stream_ = std::make_shared<UnidentifiedStream>(
            mock_recv_stream_, [this](uint64_t stream_id, uint32_t error) { error_code_ = error; },
            [this](uint64_t stream_type, std::shared_ptr<IQuicRecvStream> stream,
    ...
    void OnStreamTypeIdentified(
        ...
        receiver_stream_ = std::make_shared<PushReceiverStream>(
            qpack_encoder_, stream, [this](uint64_t stream_id, uint32_t error) { error_code_ = error; },
...
TEST_F(ServerPushRFCTest, MultiplePushIDs) {
    ...
    for (const auto& [push_id, body] : test_cases) {
        ...
        ASSERT_TRUE(sender_stream_->SendPushResponse(push_id, response)) << "Failed to send push with ID: " << push_id;

        ASSERT_NE(push_response_received_, nullptr) << "No response received for push ID: " << push_id;
        EXPECT_EQ(push_response_received_->GetBodyAsString(), body) << "Body mismatch for push ID: " << push_id;
    }
}
...
TEST_F(ServerPushRFCTest, SequentialPushIDs) {
    for (uint64_t push_id = 0; push_id < 5; ++push_id) {
        ...
        ASSERT_TRUE(sender_stream_->SendPushResponse(push_id, response)) << "Failed at push_id: " << push_id;
        ASSERT_NE(push_response_received_, nullptr);
