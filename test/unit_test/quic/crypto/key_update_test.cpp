...
static bool EncryptOnce(std::shared_ptr<ICryptographer> enc, std::shared_ptr<common::SingleBlockBuffer>& out_cipher) {
...
static ICryptographer::Result DecryptOnce(
    std::shared_ptr<ICryptographer> dec, std::shared_ptr<common::SingleBlockBuffer> cipher) {
    uint8_t aad_raw[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    common::BufferSpan aad(aad_raw, aad_raw + sizeof(aad_raw));
...
TEST(KeyUpdateTest, AES128_UpdateWriteServer_ReadClient) {
    ...
    uint8_t baseA[32];
    for (int i = 0; i < 32; ++i) baseA[i] = static_cast<uint8_t>(0x10 + i);
    uint8_t baseB[32];
    for (int i = 0; i < 32; ++i) baseB[i] = static_cast<uint8_t>(0x80 + i);
...
TEST(KeyUpdateTest, ChaCha20_UpdateWriteClient_ReadServer) {
    uint8_t baseA[32];
    for (int i = 0; i < 32; ++i) baseA[i] = static_cast<uint8_t>(0x21 + i);
    uint8_t baseB[32];
    for (int i = 0; i < 32; ++i) baseB[i] = static_cast<uint8_t>(0xB0 + i);
