...
static const uint8_t kRfcSalt[] = {0x38, 0x76, 0x2c, 0xf7, 0xf5, 0x59, 0x34, 0xb3, 0x4d, 0x17, 0x9a, 0xe6, 0xa4, 0xc8,
...
static bool RoundTripPacket(std::shared_ptr<ICryptographer> enc, std::shared_ptr<ICryptographer> dec) {
    // Build plaintext
...
static bool RoundTripHeader(std::shared_ptr<ICryptographer> enc, std::shared_ptr<ICryptographer> dec) {
    ...
    const uint8_t pn_code = 1;  // low 2-bit code stored in header (1 => 2 bytes length)
    ...
    // Expect PN length recovered correctly
...
TEST(Rfc9001InitialVectors, Aes128Gcm_InitialSecrets_RoundTrip) {
    ...
    ASSERT_EQ(srv->InstallInitSecret(kRfcDcid, sizeof(kRfcDcid), kRfcSalt, sizeof(kRfcSalt), true),
        ICryptographer::Result::kOk);
    ASSERT_EQ(cli->InstallInitSecret(kRfcDcid, sizeof(kRfcDcid), kRfcSalt, sizeof(kRfcSalt), false),
        ICryptographer::Result::kOk);
