...
bool DecryptPacketTest(std::shared_ptr<ICryptographer> encrypter, std::shared_ptr<ICryptographer> decrypter) {
    ...
    // make test plaintext
    std::shared_ptr<common::SingleBlockBuffer> plaintext =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(2048));
    ...
    std::shared_ptr<common::SingleBlockBuffer> out_ciphertext =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(2048));
    common::BufferSpan associated_data_span =
        common::BufferSpan((uint8_t*)kAssociatedData, (uint8_t*)kAssociatedData + sizeof(kAssociatedData));
    ...
    std::shared_ptr<common::SingleBlockBuffer> out_plaintext =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(2048));
    common::BufferSpan associated_data_span1 =
        common::BufferSpan((uint8_t*)kAssociatedData, (uint8_t*)kAssociatedData + sizeof(kAssociatedData));
...
bool DecryptHeaderTest(std::shared_ptr<ICryptographer> encrypter, std::shared_ptr<ICryptographer> decrypter) {
    ...
    auto src_ciphertext_span =
        common::BufferSpan((uint8_t*)src_ciphertext, (uint8_t*)src_ciphertext + sizeof(src_ciphertext));
