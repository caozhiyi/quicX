...
TEST_F(StreamIntegrationTest, CompleteRecvStreamLifecycle) {
    ...
    std::shared_ptr<common::SingleBlockBuffer> data_buffer1 =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(6));
    ...
    frame3->SetOffset(11);
    uint8_t data3[] = "!";
    std::shared_ptr<common::SingleBlockBuffer> data_buffer3 =
...
TEST_F(StreamIntegrationTest, BidirectionalRequestResponseFlow) {
    ...
    std::shared_ptr<common::SingleBlockBuffer> data_buffer = std::make_shared<common::SingleBlockBuffer>(
        std::make_shared<common::StandaloneBufferChunk>(strlen((char*)response)));
...
TEST_F(StreamIntegrationTest, StreamResetRecovery) {
    ...
    stream->SetStreamWriteCallBack([&](uint32_t size, uint32_t error) { error_received = error; });
...
TEST_F(StreamIntegrationTest, OutOfOrderDataReassembly) {
    ...
    ASSERT_TRUE(event_loop->Init());
    auto stream =
        std::make_shared<RecvStream>(event_loop, 500, 5, active_send_cb_, stream_close_cb_, connection_close_cb_);
...
TEST_F(StreamIntegrationTest, MultipleStreamsInOnePacket) {
    ...
    auto stream1 =
        std::make_shared<SendStream>(event_loop, 10000, 4, active_send_cb_, stream_close_cb_, connection_close_cb_);
    auto stream2 =
        std::make_shared<SendStream>(event_loop, 10000, 8, active_send_cb_, stream_close_cb_, connection_close_cb_);
    auto stream3 =
        std::make_shared<SendStream>(event_loop, 10000, 12, active_send_cb_, stream_close_cb_, connection_close_cb_);
...
TEST_F(StreamIntegrationTest, SendFlowControlBlocking) {
    ...
    ASSERT_TRUE(event_loop->Init());
    auto stream =
        std::make_shared<SendStream>(event_loop, 100, 4, active_send_cb_, stream_close_cb_, connection_close_cb_);
...
TEST_F(StreamIntegrationTest, RecvFlowControlAutoExpansion) {
    ...
    std::shared_ptr<common::SingleBlockBuffer> data_buffer =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(400));
...
TEST_F(StreamIntegrationTest, FinalSizeConsistencyAcrossFrames) {
    ...
    memset(data1, 'A', 50);
    std::shared_ptr<common::SingleBlockBuffer> data_buffer1 =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(50));
...
TEST_F(StreamIntegrationTest, ResetStreamAfterPartialData) {
    ...
    stream->SetStreamReadCallBack([&](std::shared_ptr<IBufferRead> buf, bool last, uint32_t err) { error_code = err; });
