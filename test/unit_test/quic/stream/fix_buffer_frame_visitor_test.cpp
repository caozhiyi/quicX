...
protected:
    void SetUp() override {
        // Prepare buffer
        buffer_ =
            std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(sizeof(buf_)));
...
TEST_F(FixBufferFrameVisitorTest, HandleCryptoFrame) {
    ...
    std::shared_ptr<common::SingleBlockBuffer> data_buffer =
        std::make_shared<common::SingleBlockBuffer>(std::make_shared<common::StandaloneBufferChunk>(11));
