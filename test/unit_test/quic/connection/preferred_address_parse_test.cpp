// Tests for BaseConnection::ParsePreferredAddress.
//
...
TEST(PreferredAddressParseTest, RejectsBareIpv6InsteadOfCrashing) {
    ...
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("::1:4433", out));
}

TEST(PreferredAddressParseTest, RejectsNonNumericPort) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("a:b", out));
}

TEST(PreferredAddressParseTest, RejectsPortOverflowingUint64) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("host:99999999999999999999", out));
}

TEST(PreferredAddressParseTest, RejectsEmptyPort) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("x:", out));
}
...
TEST(PreferredAddressParseTest, RejectsMissingColon) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1", out));
}

TEST(PreferredAddressParseTest, RejectsEmptyHost) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress(":4433", out));
}

TEST(PreferredAddressParseTest, RejectsEmptyString) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("", out));
}
...
TEST(PreferredAddressParseTest, RejectsPortAboveUint16RangeInsteadOfTruncating) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:65536", out));
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:70000", out));
}

TEST(PreferredAddressParseTest, RejectsZeroPort) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:0", out));
}

TEST(PreferredAddressParseTest, RejectsNegativePort) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:-1", out));
}

TEST(PreferredAddressParseTest, RejectsTrailingGarbageAfterPort) {
    common::Address out;
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:443x", out));
    EXPECT_FALSE(BaseConnection::ParsePreferredAddress("127.0.0.1:44 3", out));
}
...
TEST(PreferredAddressParseTest, AcceptsIpv4) {
    common::Address out;
    ASSERT_TRUE(BaseConnection::ParsePreferredAddress("127.0.0.1:4433", out));
...
TEST(PreferredAddressParseTest, AcceptsBracketedIpv6) {
    common::Address out;
    ASSERT_TRUE(BaseConnection::ParsePreferredAddress("[::1]:4433", out));
...
TEST(PreferredAddressParseTest, AcceptsFullBracketedIpv6) {
    common::Address out;
    ASSERT_TRUE(BaseConnection::ParsePreferredAddress("[2001:db8::1]:443", out));
...
TEST(PreferredAddressParseTest, AcceptsBoundaryPorts) {
    common::Address out;
    ASSERT_TRUE(BaseConnection::ParsePreferredAddress("10.0.0.1:1", out));
    EXPECT_EQ(out.GetPort(), 1);

    ASSERT_TRUE(BaseConnection::ParsePreferredAddress("10.0.0.1:65535", out));
