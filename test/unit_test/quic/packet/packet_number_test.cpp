...
TEST(packet_number_utest, codec) {
    ...
    for (size_t i = 0; i < 1000; i++) {
        packet_number.NextPacketNumber(PacketNumberSpace::kApplicationNumberSpace);
    }

    uint64_t pn = packet_number.NextPacketNumber(PacketNumberSpace::kApplicationNumberSpace);
    EXPECT_EQ(pn, 1001);
