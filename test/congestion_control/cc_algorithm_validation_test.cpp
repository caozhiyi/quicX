...
TEST(CCAlgorithmValidationTest, RenoSlowStartExponentialGrowth) {
    ...
    if (metrics.state_history.size() >= 10) {
        ...
        for (size_t i = 1; i < 10 && i < metrics.state_history.size(); ++i) {
            if (metrics.state_history[i].in_slow_start && metrics.state_history[i - 1].in_slow_start) {
                double growth_ratio =
                    static_cast<double>(metrics.state_history[i].cwnd_bytes) / metrics.state_history[i - 1].cwnd_bytes;
                ...
                if (growth_ratio > 1.0) {
                    ...
                    printf("RTT %zu: cwnd %.2f KB -> %.2f KB (ratio: %.2f)\n", i,
                        metrics.state_history[i - 1].cwnd_bytes / 1024.0, metrics.state_history[i].cwnd_bytes / 1024.0,
                        growth_ratio);
                }
            }
        }
...
TEST(CCAlgorithmValidationTest, RenoCwndReductionOnLoss) {
    ...
    for (size_t i = 1; i < metrics.state_history.size(); ++i) {
        if (!metrics.state_history[i - 1].in_recovery && metrics.state_history[i].in_recovery) {
            // Entering recovery
            ...
            if (cwnd_after < max_cwnd_before_recovery) {
                ...
                printf("Cwnd reduction: %.2f KB -> %.2f KB (ratio: %.2f)\n", max_cwnd_before_recovery / 1024.0,
                    cwnd_after / 1024.0, reduction_ratio);
...
TEST(CCAlgorithmValidationTest, CubicConcaveConvexBehavior) {
    ...
    if (found_recovery_exit && recovery_exit_idx + 10 < metrics.state_history.size()) {
        ...
        for (size_t i = recovery_exit_idx + 1; i < recovery_exit_idx + 10 && i < metrics.state_history.size(); ++i) {
...
TEST(CCAlgorithmValidationTest, RTTMeasurementAccuracy) {
...
TEST(CCAlgorithmValidationTest, RTTVariationTracking) {
...
TEST(CCAlgorithmValidationTest, SsthreshUpdateOnLoss) {
    ...
    for (size_t i = 1; i < metrics.state_history.size(); ++i) {
        if (metrics.state_history[i - 1].in_slow_start && !metrics.state_history[i].in_slow_start) {
...
TEST(CCAlgorithmValidationTest, BytesInFlightReasonable) {
    ...
    for (const auto& snapshot : metrics.state_history) {
        if (snapshot.bytes_in_flight > snapshot.cwnd_bytes * 3) {
            printf("Large excess: bytes_in_flight=%.2f KB > 3*cwnd=%.2f KB\n", snapshot.bytes_in_flight / 1024.0,
                (snapshot.cwnd_bytes * 3) / 1024.0);
            violations++;
        }
    }
...
TEST(CCAlgorithmValidationTest, ZeroLossIdealConditions) {
...
TEST(CCAlgorithmValidationTest, VeryLowBandwidth) {
...
TEST(CCAlgorithmValidationTest, VeryHighLatency) {
...
TEST(CCAlgorithmValidationTest, MultipleRecoveryPeriods) {
...
TEST(CCAlgorithmValidationTest, CwndConvergenceAfterLoss) {
...
TEST(CubicOptimizationTest, FastConvergenceOnRepeatedLoss) {
    ...
    for (const auto& snapshot : metrics.state_history) {
        if (snapshot.time_us < 7500000) {
            max_cwnd_first_half = std::max(max_cwnd_first_half, static_cast<double>(snapshot.cwnd_bytes));
