            ...
            if (last_probe_rtt_time == 0 ||
                ...
                if (last_probe_rtt_time > 0) {
                    ...
                    printf("ProbeRTT #%d at %.2fs (cwnd: %.2f->%.2f KB), interval: %.2fs\n", probe_rtt_count,
                        metrics.state_history[i].time_us / 1e6, prev_cwnd / 1024.0, curr_cwnd / 1024.0, interval / 1e6);
                }
...
TEST(BBRv2DetailedTest, ProbeRTTCwndReduction) {
    ...
    for (size_t i = 1; i < metrics.state_history.size(); i++) {
        ...
        if (!in_probe_rtt && prev_cwnd > 10 * 1460 && curr_cwnd < prev_cwnd * 0.4 && curr_cwnd < 8 * 1460) {
            in_probe_rtt = true;
...
TEST(BBRv1DetailedTest, ProbeRTTBehavior) {
    ...
    for (size_t i = 1; i < metrics.state_history.size(); i++) {
        ...
        if (prev_cwnd > 10 * 1460 && curr_cwnd < prev_cwnd * 0.4 && curr_cwnd < 8 * 1460) {
            // Avoid counting consecutive drops
            if (last_probe_rtt_time == 0 || metrics.state_history[i].time_us - last_probe_rtt_time > 5000000) {
                probe_rtt_count++;
                last_probe_rtt_time = metrics.state_history[i].time_us;
                printf("BBRv1 ProbeRTT #%d at %.2fs (cwnd: %.2f->%.2f KB)\n", probe_rtt_count,
                    metrics.state_history[i].time_us / 1e6, prev_cwnd / 1024.0, curr_cwnd / 1024.0);
            }
        }
    }
...
TEST(BBRv2DetailedTest, MultiInstanceNoInterference) {
    ...
    printf("Instance 1 (Fast): Throughput=%.2f Mbps, SlowStart=%.3fs\n", metrics1.throughput_mbps,
        metrics1.slow_start_duration_us / 1e6);
    printf("Instance 2 (Slow): Throughput=%.2f Mbps, SlowStart=%.3fs\n", metrics2.throughput_mbps,
        metrics2.slow_start_duration_us / 1e6);
    printf("Instance 3 (Med):  Throughput=%.2f Mbps, SlowStart=%.3fs\n", metrics3.throughput_mbps,
        metrics3.slow_start_duration_us / 1e6);
    ...
    for (size_t i = 1; i < metrics.state_history.size(); i++) {
        ...
        if (prev_cwnd > 20 * 1460 && curr_cwnd < prev_cwnd * 0.8) {
            ...
            printf("Cwnd reduction at %.2fs: %.2f KB -> %.2f KB (ratio: %.2f)\n",
                metrics.state_history[i].time_us / 1e6, prev_cwnd / 1024.0, curr_cwnd / 1024.0, reduction_ratio);
        }
...
TEST(BBRStabilityTest, LongRunStability_60s) {
    ...
    printf("BBRv1: Throughput=%.2f Mbps, Loss=%.2f%%, Avg RTT=%.2f ms\n", metrics_v1.throughput_mbps,
        metrics_v1.packet_loss_rate * 100, metrics_v1.avg_rtt_ms);
    printf("BBRv2: Throughput=%.2f Mbps, Loss=%.2f%%, Avg RTT=%.2f ms\n", metrics_v2.throughput_mbps,
        metrics_v2.packet_loss_rate * 100, metrics_v2.avg_rtt_ms);
...
TEST(BBRv2DetailedTest, RecoverySpeedAfterLoss) {
    ...
    for (const auto& state : metrics.state_history) {
        if (state.time_us > 10000000 && state.cwnd_bytes >= target_cwnd * 0.8) {
