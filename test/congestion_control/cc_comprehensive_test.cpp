...
CCTestMetrics RunTest(const std::string& algo_name, CCTestFramework::CCFactory factory, const TestScenario& scenario,
...
TEST(CCBehaviorTest, RenoSlowStartInIdealNetwork) {
    auto metrics = RunTest("Reno", CCAlgorithmFactory::Reno(), TestScenario::IdealNetwork(3000000), true);
...
TEST(CCBehaviorTest, RenoLossRecovery) {
    auto metrics = RunTest("Reno", CCAlgorithmFactory::Reno(), TestScenario::LossyNetwork(), true);
...
TEST(CCBehaviorTest, CubicInMobileNetwork) {
    auto metrics = RunTest("CUBIC", CCAlgorithmFactory::CUBIC(), TestScenario::MobileNetwork(), true);
...
TEST(CCBehaviorTest, BBRv1InBufferBloat) {
    auto metrics = RunTest("BBRv1", CCAlgorithmFactory::BBRv1(), TestScenario::BufferBloat(), true);
...
TEST(CCDynamicTest, NetworkDegradation_Reno) {
    auto metrics = RunTest("Reno", CCAlgorithmFactory::Reno(), TestScenario::NetworkDegradation(), true);
...
TEST(CCDynamicTest, NetworkDegradation_CUBIC) {
    auto metrics = RunTest("CUBIC", CCAlgorithmFactory::CUBIC(), TestScenario::NetworkDegradation(), true);
...
TEST(CCDynamicTest, FluctuatingNetwork) {
    auto metrics = RunTest("Reno", CCAlgorithmFactory::Reno(), TestScenario::FluctuatingNetwork(), true);
...
TEST(CCRegressionTest, RenoMinimumPerformance) {
    auto metrics = RunTest("Reno", CCAlgorithmFactory::Reno(), TestScenario::LowLatencyNetwork());
...
TEST(CCRegressionTest, CubicMinimumPerformance) {
    auto metrics = RunTest("CUBIC", CCAlgorithmFactory::CUBIC(), TestScenario::LowLatencyNetwork());
...
static void PrintComparisonTable(const std::vector<AlgoResult>& results) {
    ...
    for (const auto& r : results) {
        printf("%-10s | %7.2f Mbps | %7.2f KB | %8.2f%% | %7.2f ms\n", r.name.c_str(), r.metrics.throughput_mbps,
            r.metrics.max_cwnd_bytes / 1024.0, r.metrics.packet_loss_rate * 100.0, r.metrics.avg_rtt_ms);
    }
}
...
TEST(CCComprehensiveTest, AllScenariosAlgorithmComparisonParallel) {
    ...
    auto add = [&scenarios](const std::string& name, const NetworkCondition& nc, uint64_t dur) {
        TestScenario s;
        s.name = name;
        s.network_condition = nc;
        s.test_duration_us = dur;
