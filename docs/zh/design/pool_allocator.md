...
- **`Poolallocator`**（`src/common/allocator/`）—— 小对象 slab free-list 分配器（≤256B），**per-connection、单线程、连接寿命内只增不还**。设计上为"未来要把 frame 这类小对象池化"留好接口，但 *目前尚未接入产品代码*（决策见 `pool_allocator_frame_optimization.md`）。
- **`BlockMemoryPool`**（`src/common/allocator/pool_block.h`）—— 1500B 大块池，`thread_local` per worker，**真正承担流量**：每个 QUIC 数据报的明文/密文缓冲、每条 stream 的发送缓冲、`NetPacket` 的 `IBuffer`，最终都由它发块。
...
- `Iallocator` 接口 + `allocatorWrap` + `PoolUniquePtr<T>` 这一组工具准备好了什么、避免了什么；
- 为什么"小对象 slab"最自然的目标 —— frame —— *目前并没有接* `Poolallocator`，与 `pool_allocator_frame_optimization.md` 的暂缓决策怎么对上；
- 三套池在跨线程 / 关闭顺序 / 失败模式上的不变量。

阅读时建议打开 `src/common/allocator/`、`src/common/buffer/buffer_chunk_pool.{h,cpp}`、`src/quic/quicx/global_resource.{h,cpp}` 这三处源码。
...
flowchart TB
    ...
    subgraph 应用层["应用层（被池服务的对象）"]
        FRAME["frame 对象（StreamFrame / AckFrame …）<br/>当前走 std::make_shared，未接 Poolallocator"]:::notyet
    ...
    subgraph 工具层["allocatorWrap 工具层（建好但当前未广泛接入）"]
        WRAP["allocatorWrap<br/>+ PoolUniquePtr&lt;T&gt;<br/>+ PoolDeleter&lt;T&gt;（无状态、16B）"]:::bridge
    end

    subgraph 池层["三套池"]
        POOL_A["Poolallocator<br/>（slab free-list, ≤256B）<br/>per-connection / 单线程 / 只增不还"]:::pool
    ...
    subgraph 底层["底层"]
        NORMAL["Normalallocator（malloc/free 直通）"]:::sink
...
1. **三套池服务的目标尺寸不同**：`Poolallocator` 锁定 ≤256B 的小对象；`BlockMemoryPool` 锁定 1500B 的大块（紧贴以太网 MTU）；`BufferChunkPool` 回收的是 BufferChunk *wrapper 对象本身*（约 80B 的元数据，块仍由 `BlockMemoryPool` 管）。
2. **生命周期 / 线程性 / 缩容策略各不相同**：`Poolallocator` 是 per-connection 单线程、连接寿命内只增不还；`BlockMemoryPool` 是 per-worker thread_local、跨线程 free 走 `RunInLoop`、超过 `kMaxBlockNum` 就 `ReleaseHalf` 主动收缩；`BufferChunkPool` 是 thread_local + 256 上限的简单 free list，溢出就直接 `delete`（其内部块顺势归还给 `BlockMemoryPool`）。
3. **`allocatorWrap` 与 `PoolUniquePtr<T>` 是已铺好的"零开销 RAII 桥"**，但产品代码里**还没有把 frame 接上去** —— 这不是疏忽，而是 `pool_allocator_frame_optimization.md` 已经做完的成本/风险评估的结论。
4. **真正承担流量的是 `BlockMemoryPool`**：每个出向 QUIC 数据报、每个入向 NetPacket、每条 stream 的发送/接收缓冲，最终都从它发 1500B 块出去。`Poolallocator` 设计虽精巧，但在当前栈里更像"留好的位置"。

---

## 2. `Iallocator` 接口与 `allocatorWrap` 工具

源码：`src/common/allocator/if_allocator.h`。`Iallocator` 是分配器抽象（`Malloc / MallocAlign / MallocZero / Free`）；`allocatorWrap` 是面向使用方的便利层。
...
- `Free` 第二参数 `len`：`Poolallocator` 释放小对象时**必须知道原始 size** 才能定位 free-list bucket（`FreeListIndex(len)`）；`len = 0` 被显式当作"我不知道大小，请走 fallback"处理（直接转给 `Normalallocator`），避免 `(0 + 7) / 8 - 1` 下溢成 `UINT32_MAX` 导致 vector 越界。
...
template<typename T>
class PoolDeleter {
    Iallocator* allocator_;          // 一个裸指针，无 atomic refcount
public:
    void operator()(T* p) const noexcept {
        if (!p) return;
        p->~T();
        void* data = static_cast<void*>(p);
        allocator_->Free(data, sizeof(T));   // sizeof(T) 提供 len，Free 自然走 free-list
    }
};
...
`sizeof(PoolUniquePtr<T>)` = 16B（一个 `T*` + 一个 `Iallocator*`）。比 `shared_ptr<T>` 的 16B（指针 + 控制块指针）小一档：**没有控制块、没有 atomic inc/dec**。

### 2.3 已过期的 `PoolNewSharePtr`

`allocatorWrap::PoolNewSharePtr` / `PoolMallocSharePtr` 都标了 `[[deprecated]]`，理由写得很直白：
...
`kAlign = sizeof(unsigned long)`（64-bit Linux 上 8B）。`Poolallocator` 的 free-list bucket 也是按 `kAlign` 步进切片：
...
## 3. `Poolallocator` —— 小对象 slab（已建好、暂未接入）

源码：`src/common/allocator/pool_allocator.{h,cpp}`，约 150 行。算法继承自经典 STL slab allocator（`__gnu_cxx::__pool_alloc` 类似思路）。
...
private:
    ...
    std::vector<uint8_t*>     malloc_vec_;  // 历史申请过的所有大块（用于 ~Poolallocator 一次释放）
    std::shared_ptr<Iallocator> allocator_;     // 后端 = Normalallocator
};
```

**为什么 `free_list` 是 32 桶**：`kDefaultMaxBytes = 256`、`kAlign = 8`，所以 `[8B, 16B, 24B, …, 256B]` 共 32 个 bucket。`>256B` 的请求**直接转后端 `Normalallocator`**（`Malloc` / `Free` 都有这条 fallback）—— `Poolallocator` 只负责 *小* 对象。
...
```text
size > 256B   →  转 Normalallocator（malloc 直通）
...
```text
data == nullptr     →  no-op
len == 0 || > 256B  →  转 Normalallocator（防御 + fallback）
...
- 够切 20 块 → 一次切 20 块；
- 只够切 1~19 块 → 切多少要多少；
- 完全不够 → 把剩余尾段挂到对应 bucket（避免内部碎片浪费），然后向 `Normalallocator` 申请新的 `20 * size` 字节大块，递归再切。

新申请的大块都登记到 `malloc_vec_`，析构时一次性 `allocator_->Free` 全归还系统。

### 3.3 三个不变量

1. **单线程独占**：`Poolallocator` *没有任何锁*，文档写明 "intentionally NOT thread-safe"。设计模型是 "one Poolallocator per Connection"，连接被 framework 钉到单一 I/O 线程上。
2. **只增不还**：连接寿命内 `malloc_vec_` 只增长，`Free` 把节点退回 free-list 但**不归还系统**。这与连接级"用完就丢整片 arena"的语义吻合 —— 连接关闭那一刻 `~Poolallocator` 把所有大块一把还给 glibc。
3. **`Free` 必须传准确 `len`**：调用方有义务记得对象原始大小（这正是 `PoolDeleter::operator()` 用 `sizeof(T)` 而不是 0 的原因）。

### 3.4 `Poolallocator` 在产品代码中的接入现状
...
## 4. 为什么 frame 没有走 `Poolallocator`：暂缓决策摘要

完整决策剧本在 `docs/internal/pool_allocator_frame_optimization.md`（约 250 行）。这里把 *与设计折衷相关* 的结论压缩成一节，避免读者读完本文以为"Poolallocator 是孤儿"：

| 维度 | 结论 |
| :--- | :--- |
| **技术可行性** | ✅ 可行。`PoolDeleter` / `PoolUniquePtr` / `PoolMakeUnique` 已就位（`if_allocator.h` 内）。 |
...
> **Take-away**：`Poolallocator` 不是"被废弃"，而是"被论证过、暂缓接入"。本文与 `pool_allocator_frame_optimization.md` 的分工是 —— 本文讲 *机制*，那篇讲 *决策*。

---

## 5. `BlockMemoryPool` —— 1500B 大块池（真·主力）

源码：`src/common/allocator/pool_block.{h,cpp}` + `src/quic/quicx/global_resource.{h,cpp}`。
...
## 7. `PoolPacketallocator` —— `NetPacket` 对象池

源码：`src/quic/udp/pool_packet_allocator.{h,cpp}`。这是基于 `BlockMemoryPool` 的更高层包装：
...
Malloc():
    if packet_queue_.Pop(pkt):
        ...
        return NormalPacketallocator::Malloc()    // 池空了 fallback
...
`PoolPacketallocator` 是 `packet_lifecycle.md` §2.4 提到的 "`PoolPacketallocator`，底层 chunk 来自 `common::BlockMemoryPool`" 的具体实现。

---

## 8. 三套池的横向对照

| 维度 | `Poolallocator` | `BlockMemoryPool` | `BufferChunkPool` | `PoolPacketallocator` |
| :--- | :--- | :--- | :--- | :--- |
...
| **失败 / 超尺寸** | `>256B` 转 `Normalallocator` | `Expansion` 失败记日志、跳过 | 块分配失败 → 返回 `nullptr` | 队列空 → 退到 `NormalPacketallocator` |
| **接入度** | **0 处产品代码**（仅工具就位） | 高（packet / buffer / send_buffer 都用） | 高（每 packet plaintext 路径） | 高（receiver hot path） |
| **metrics** | 无 | `MemPool*` 系列 4 个 | 无 | 无 |
| **关联设计文档** | 本文 + `pool_allocator_frame_optimization.md` | `packet_lifecycle.md`、`metrics.md` | `packet_lifecycle.md` §2.4 | `packet_lifecycle.md` |

横向看下来，**三套池确实不能合**：

- `Poolallocator` 和 `BlockMemoryPool` 尺寸量级差 6 倍（256B vs 1500B），且 free-list 策略（多桶 @8B 步进）和 single-size 策略（1500B 唯一）天然互斥。
- `BufferChunkPool` 必须建在 `BlockMemoryPool` *之上*：它管的是"装载块的对象"，本身仍要从 `BlockMemoryPool` 取块。
- `PoolPacketallocator` 跨线程，与 `BlockMemoryPool` 的 thread_local 模型互补 —— receiver 线程从池取空 packet → worker 线程消费 → 释放时回队列。
...
- **`Poolallocator::Free` 必须传准确 `len`**，否则 `len=0` 走 fallback、`len>256` 也走 fallback —— 永远不会被 free-list 吃错桶。
- **`Poolallocator` 单线程独占 / 不缩容** —— 配合 "one allocator per Connection" 模型；一旦未来重启 frame 池化，**禁止跨线程共享 allocator 实例**。
...
- **`PoolPacketallocator` 满则 delete** —— 不允许 burst 让对象池无限膨胀；超量请求自然回落到 `NormalPacketallocator::Malloc`。
...
- [`packet_lifecycle.md`](packet_lifecycle.md) —— `NetPacket::buffer_` 的来源（`PoolPacketallocator` + `BlockMemoryPool`），§2.4 与本文 §7 互看。
