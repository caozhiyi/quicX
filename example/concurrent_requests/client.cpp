...
public:
    ...
    void PrintResults() {
        ...
        std::cout << std::left 
                  << std::setw(6) << "ID"
                  << std::setw(15) << "Endpoint"
                  << std::setw(10) << "Status"
                  << std::setw(12) << "Duration(ms)"
                  << std::setw(12) << "Start(ms)"
                  << std::setw(12) << "End(ms)"
                  << std::endl;
        std::cout << std::string(80, '-') << std::endl;

        for (const auto& result : results_) {
            auto start_offset = std::chrono::duration_cast<std::chrono::milliseconds>(
                result.start_time - first_start);
            auto end_offset = std::chrono::duration_cast<std::chrono::milliseconds>(
                result.end_time - first_start);
            
            std::cout << std::left
                      << std::setw(6) << result.id
                      << std::setw(15) << result.endpoint
                      << std::setw(10) << (result.success ? std::to_string(result.status_code) : "Error")
                      << std::setw(12) << result.duration_ms
                      << std::setw(12) << start_offset.count()
                      << std::setw(12) << end_offset.count()
                      << std::endl;
        }
        ...
        std::cout << "Avg Request Duration: " << std::fixed << std::setprecision(2) 
        ...
        std::cout << std::string(80, '-') << std::endl;
        std::cout << "Parallelization:      " << std::fixed << std::setprecision(1) 
                  << efficiency << "%" << std::endl;
    ...
    void PrintTimeline(const std::chrono::steady_clock::time_point& first_start, int64_t total_ms) {
        ...
        for (const auto& result : results_) {
            auto start_offset = std::chrono::duration_cast<std::chrono::milliseconds>(
                result.start_time - first_start).count();
            auto end_offset = std::chrono::duration_cast<std::chrono::milliseconds>(
                result.end_time - first_start).count();
