...
struct RequestStats {
    ...
    void DecrementConcurrent() { --concurrent_requests; }
    ...
    std::string GetStatsJson() {
        ...
        oss << "{"
            << "\"total_requests\":" << total_requests.load() << ",\"fast_requests\":" << fast_requests.load()
            << ",\"medium_requests\":" << medium_requests.load() << ",\"slow_requests\":" << slow_requests.load()
            << ",\"current_concurrent\":" << concurrent_requests.load()
            << ",\"max_concurrent\":" << max_concurrent.load() << ",\"uptime_seconds\":" << duration.count()
...
public:
    RAII_ConcurrentCounter(RequestStats& stats):
        stats_(stats) {
        stats_.IncrementConcurrent();
    }
    ~RAII_ConcurrentCounter() { stats_.DecrementConcurrent(); }
};
...
int main() {
    ...
    server->AddMiddleware(quicx::HttpMethod::kAny, quicx::MiddlewarePosition::kBefore,
        [](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
            std::cout << "[" << GetCurrentTime() << "] [" << req->GetMethodString() << "] " << req->GetPath()
                      << std::endl;
        });
    ...
    server->AddHandler(quicx::HttpMethod::kGet, "/fast",
        [stats](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
    ...
    server->AddHandler(quicx::HttpMethod::kGet, "/medium",
        [stats](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
            RAII_ConcurrentCounter counter(*stats);
            stats->total_requests++;
    ...
    server->AddHandler(quicx::HttpMethod::kGet, "/random",
        [stats](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
            RAII_ConcurrentCounter counter(*stats);
            stats->total_requests++;
            ...
            oss << "{\"endpoint\":\"random\",\"delay_ms\":" << delay_ms
                << ",\"message\":\"Random delay response\",\"timestamp\":\"" << GetCurrentTime() << "\"}";
    ...
    server->AddHandler(quicx::HttpMethod::kGet, "/data/:size",
        [stats](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
            RAII_ConcurrentCounter counter(*stats);
            stats->total_requests++;
            ...
            header << "{\"size_kb\":" << size_kb << ",\"size_bytes\":" << data.size() << ",\"timestamp\":\""
                   << GetCurrentTime() << "\"}\n\n";
