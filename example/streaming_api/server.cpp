...
public:
    void OnHeaders(std::shared_ptr<IRequest> request, std::shared_ptr<IResponse> response) {
        ...
        std::cout << "[Upload] Receiving file: " << filename << std::endl;

        // Get Content-Length if available for progress tracking
        std::string content_length_str;
        if (request->GetHeader("Content-Length", content_length_str)) {
            try {
                total_size_ = std::stoull(content_length_str);
                std::cout << "[Upload] File size: " << total_size_ << " bytes" << std::endl;
            } catch (...) {
    ...
    void OnBodyChunk(const uint8_t* data, size_t length, bool is_last) {
        ...
        if (file_ && length > 0) {
            ...
            if (written != length) {
                std::cerr << "[Upload] Write error: expected " << length << " bytes, wrote " << written << std::endl;
            }
 ...
 * using body_provider.
 */
void HandleFileDownload(std::shared_ptr<IRequest> request, std::shared_ptr<IResponse> response) {
    std::string filename;
    auto& path_params = request->GetPathParams();
    auto it = path_params.find("filename");
    if (it != path_params.end()) {
 ...
 */
void HandleStatus(std::shared_ptr<IRequest> request, std::shared_ptr<IResponse> response) {
...
int main(int argc, char* argv[]) {
    ...
    g_server->AddHandler(HttpMethod::kPost, "/upload/:filename", std::make_shared<FileUploadHandler>());

    // Complete mode with body provider: File download with streaming response body
    g_server->AddHandler(HttpMethod::kGet, "/download/:filename", HandleFileDownload);
