...
int main() {
    ...
    server->AddHandler(quicx::HttpMethod::kGet, "/qlog",
        [](std::shared_ptr<quicx::IRequest> req, std::shared_ptr<quicx::IResponse> resp) {
            std::cout << "[Server] Handling request for /qlog" << std::endl;
            resp->AppendBody(std::string("QLog is active! Check ./qlog_output directory."));
    ...
    server->Init(config);
    if (!server->Start("0.0.0.0", 7012)) {
        std::cout << "[Server] Start failed" << std::endl;
        return 1;
    }

    std::cout << "[Server] Listening on 0.0.0.0:7012" << std::endl;
    std::cout << "[Server] Qlog enabled at ./qlog_output" << std::endl;
