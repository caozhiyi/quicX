...
bool HttpClient::DoRequest(const std::string& url, const std::string& method, const std::vector<std::string>& headers,
    ...
    if (!data.empty() && (method == "POST" || method == "PUT")) {
        ...
        if (!request->GetHeader("content-type", content_type) && !request->GetHeader("Content-Type", content_type)) {
    ...
    bool request_completed = false;
    {
        ...
        client_->DoRequest(url, StringToMethod(method), request,
            [this, &response, &request_completed](std::shared_ptr<quicx::IResponse> resp, uint32_t error) {
                ...
                if (error == 0 && resp) {
                    ...
                    response.body = resp->GetBodyAsString();
                }
