...
struct RouteConfig {
    ...
    explicit RouteConfig(const http_handler& h):
        handler(h) {}

    // Constructor for async mode handler
    explicit RouteConfig(std::shared_ptr<IAsyncServerHandler> h):
        handler(h) {}

    // Constructor for async client handler
    explicit RouteConfig(std::shared_ptr<IAsyncClientHandler> h):
        handler(h) {}
     ...
     */
    bool IsAsync() const { return IsAsyncClient() || IsAsyncServer(); }

    // Check if this route uses async client handler
    bool IsAsyncClient() const { return std::holds_alternative<std::shared_ptr<IAsyncClientHandler>>(handler); }

    // Check if this route uses async server handler
    bool IsAsyncServer() const { return std::holds_alternative<std::shared_ptr<IAsyncServerHandler>>(handler); }
 ...
 * 1. normal static path. like "/department/user/info", when user request "/department/user/info", it will match this
 * route.
 ...
 * if add route "/department/user/info" and "/department/user/:id", when user request "/department/user/info", it will
 * match "/department/user/info". if add route "/department/user/:id" and "/department/user/\*", when user request
 * "/department/user/info", it will match "/department/user/:id".
 *
 * 2. try to match longest path, for example:
 * if add route "/department/user/info" and "/department/user/:info/:id", when user request "/department/user/info/1",
 * it will match "/department/user/:info/:id". if add route "/department/user/\*" and "/department/user/info/:id", when
 * user request "/department/user/info/1", it will match "/department/user/info/:id".
 *
 * THINKING:
 * 1. handler is associated with http, if it can implement as a templete type, it will be more flexible and easy to
 * test, but in c++, templete class declaration and implementation must be in the same file, so it will encounter
 * circular reference problem when dynamic create subclass instance in parents class.
 */
class IRouter {
public:
    IRouter() {}
     ...
     */
    virtual bool AddRoute(HttpMethod method, const std::string& path, const RouteConfig& config) = 0;
