...
bool DecodeFrames(std::shared_ptr<common::IBuffer> buffer, std::vector<std::shared_ptr<IFrame>>& frames) {
    ...
    while (buffer->GetDataLength() > 0) {
        ...
        common::LOG_DEBUG("DecodeFrames: decoded frame type=%u (0x%04x), remaining=%u", frame_type, frame_type,
        ...
        // Flush frame type before calling frame->Decode to avoid stale buffer pointer
        wrapper.Flush();

        std::shared_ptr<IFrame> frame = creator->second();
        uint32_t length_before_decode = buffer->GetDataLength();
        DecodeResult decode_result = frame->Decode(buffer, false);
        if (decode_result == DecodeResult::kError) {
            common::LOG_ERROR(
                "DecodeFrames: failed to decode frame type=%u, remaining=%u", frame_type, buffer->GetDataLength());
            return false;
        }

        if (decode_result == DecodeResult::kNeedMoreData) {
            // Frame type already flushed, but we need to restore it
            // Move buffer pointer back by 2 bytes (frame type size)
            buffer->MoveReadPt(-2);
            common::LOG_DEBUG("DecodeFrames: need more data to decode frame type=%u", frame_type);
            return true;
        }
        ...
        common::LOG_DEBUG(
            "DecodeFrames: successfully decoded frame type=%u, "
            "consumed=%u bytes, remaining=%u",
        ...
        if (buffer->GetDataLength() >= length_before) {
            common::LOG_ERROR(
                "DecodeFrames: no progress made decoding frame "
                "type=%u, length_before=%u, length_after=%u",
    ...
    common::LOG_DEBUG(
        "DecodeFrames: completed, decoded %zu frames, consumed %u bytes total", frames.size(), initial_length);
    return true;
}
