...
void ResponseStream::SendPushPromise(const std::unordered_map<std::string, std::string>& headers, int32_t push_id) {
    ...
    if (!qpack_encoder_->Encode(headers, headers_buffer)) {
        LOG_ERROR("ResponseStream::SendPushPromise qpack encode error");
        if (error_handler_) {
...
bool ResponseStream::SendResponse(std::shared_ptr<IResponse> response) {
    ...
    LOG_DEBUG("ResponseStream::SendResponse: status=%d, body size=%zu", response->GetStatusCode(), body_size);
    ...
    if (!SendHeaders(response->GetHeaders())) {
        LOG_ERROR("ResponseStream::SendResponse: SendHeaders failed");
        return false;
    }
    LOG_DEBUG("ResponseStream::SendResponse: headers sent successfully");

    // For HEAD requests, never send body regardless of response content
    if (is_head_request) {
        stream_->Close();
        LOG_DEBUG("ResponseStream::SendResponse: HEAD request, no body sent, stream closed with FIN");
    ...
    if (provider) {
        ...
        LOG_DEBUG("ResponseStream::SendResponse: using body provider");
        return SendBodyWithProvider(provider);

    } else if (body && body_size > 0) {
        // Complete mode: send entire body
        LOG_DEBUG("ResponseStream::SendResponse: sending body directly, size=%zu", body_size);
    ...
    stream_->Close();
    LOG_DEBUG("ResponseStream::SendResponse: no body to send, stream closed with FIN");
...
void ResponseStream::HandleHeaders() {
    ...
    if (settings_received_cb_ && !settings_received_cb_()) {
        LOG_WARN("ResponseStream: Request received before peer SETTINGS frame, proceeding with defaults");
    }
    ...
    if (auto proc = http_processor_.lock()) {
        ...
        LOG_WARN("ResponseStream::HandleHeaders: http_processor expired");
        if (error_handler_) {
            error_handler_(GetStreamID(), Http3ErrorCode::kInternalError);
        }
        return;
    ...
    LOG_DEBUG("HandleHeaders: complete handler called, body_length_: %u, has_content_length: %d", body_length_,
        has_content_length);
    ...
    if (!has_content_length || body_length_ == 0) {
        ...
        HandleResponse();
        LOG_DEBUG("HandleHeaders: complete handler called and response sent");
    ...
    LOG_DEBUG("HandleHeaders: complete handler called");
...
void ResponseStream::HandleData(const std::shared_ptr<common::IBuffer>& data, bool is_last) {
    ...
    if ((body_length_ == body_->GetDataLength() || is_last) && !is_response_sent_) {
        ...
        LOG_DEBUG("ResponseStream::HandleData: calling handler and sending response");
        HandleHttp(route_config_.GetCompleteHandler());
        HandleResponse();
