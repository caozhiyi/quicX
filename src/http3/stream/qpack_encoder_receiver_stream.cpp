...
void QpackEncoderReceiverStream::OnData(std::shared_ptr<common::IBufferRead> data, uint32_t error) {
    if (error != 0) {
...
void QpackEncoderReceiverStream::ParseEncoderInstructions(std::shared_ptr<common::IBufferRead> data) {
    ...
    if (!encoder.DecodeEncoderInstructions(data)) {
