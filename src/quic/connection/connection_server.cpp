...
ServerConnection::ServerConnection(std::shared_ptr<TLSCtx> ctx, std::shared_ptr<ISender> sender,
    std::shared_ptr<common::IEventLoop> loop, const std::string& alpn,
    ...
    BaseConnection(StreamIDGenerator::StreamStarter::kServer, false, sender, loop, active_connection_cb,
        handshake_done_cb, add_conn_id_cb, retire_conn_id_cb, connection_close_cb),
    server_alpn_(alpn) {
