...
enum QuicErrorCode : uint32_t {
    kNoError = 0x00,        // an endpoint uses this with CONNECTION_CLOSE to signal that the connection is being closed
    ...
    kStreamLimitError = 0x04,  // received a frame for a stream identifier that exceeded its advertised stream limit for
    ...
    kFinalSizeError =
        0x06,  // received a STREAM frame containing data that exceeded the previously established final size.
    kFrameEncodingError = 0x07,  // received a frame that was badly formatted.
    kTransportParameterError =
        0x08,  // received transport parameters that were badly formatted, included an invalid value.
    kConnectionIdLimitError =
        0x09,  // connection IDs provided by the peer exceeds the advertised active_connection_id_limit.
