...
bool SerializedSessionData::Serialize(std::ofstream& file) const {
    ...
    file.write(reinterpret_cast<const char*>(&info.initial_max_stream_data_bidi_local),
        sizeof(info.initial_max_stream_data_bidi_local));
    file.write(reinterpret_cast<const char*>(&info.initial_max_stream_data_bidi_remote),
        sizeof(info.initial_max_stream_data_bidi_remote));
    file.write(
        reinterpret_cast<const char*>(&info.initial_max_stream_data_uni), sizeof(info.initial_max_stream_data_uni));
    file.write(
        reinterpret_cast<const char*>(&info.active_connection_id_limit), sizeof(info.active_connection_id_limit));
...
bool SerializedSessionData::Deserialize(std::ifstream& file) {
    ...
    if (version >= 2) {
        ...
        file.read(reinterpret_cast<char*>(&info.initial_max_stream_data_bidi_local),
            sizeof(info.initial_max_stream_data_bidi_local));
        file.read(reinterpret_cast<char*>(&info.initial_max_stream_data_bidi_remote),
            sizeof(info.initial_max_stream_data_bidi_remote));
...
bool SessionCache::StoreSession(const std::string& session_der, const SessionInfo& session_info) {
    ...
    if (prev != sessions_cache_.end()) {
        if (!merged_info.has_transport_params && prev->second.has_transport_params) {
            ...
            merged_info.initial_max_stream_data_bidi_local = prev->second.initial_max_stream_data_bidi_local;
            merged_info.initial_max_stream_data_bidi_remote = prev->second.initial_max_stream_data_bidi_remote;
            merged_info.initial_max_stream_data_uni = prev->second.initial_max_stream_data_uni;
            merged_info.active_connection_id_limit = prev->second.active_connection_id_limit;
            LOG_DEBUG(
                "StoreSession: preserved remembered TP from prior entry "
                "for %s (incoming had no TP)",
                session_info.server_name.c_str());
        }
    }
    ...
    LOG_DEBUG("Stored session for %s, timeout: %u, early_data_capable: %d, has_tp: %d, cache size: %zu",
        merged_info.server_name.c_str(), merged_info.timeout, merged_info.early_data_capable,
        merged_info.has_transport_params, sessions_cache_.size());
...
bool SessionCache::GetSession(const std::string& server_name, std::string& out_session_der) {
    ...
    LOG_DEBUG("Retrieved valid session for %s, remaining lifetime: %u seconds", server_name.c_str(),
        GetSessionRemainingLifetime(info));
    return true;
}

bool SessionCache::GetSessionWithInfo(
    const std::string& server_name, std::string& out_session_der, SessionInfo& out_info) {
    if (!enable_session_cache_) {
        return false;
    }
    ...
    LOG_DEBUG("Retrieved session with info for %s, has_tp=%d (mem=%d, disk=%d), remaining: %u seconds",
        server_name.c_str(), out_info.has_transport_params, it->second.has_transport_params,
        data.info.has_transport_params, GetSessionRemainingLifetime(info));
    return true;
}
...
bool SessionCache::HasValidSessionFor0RTT(const std::string& server_name) {
    ...
    if (!can_use) {
        LOG_DEBUG("Session for %s cannot be used for 0-RTT (expired: %d, early_data_capable: %d)", server_name.c_str(),
            !IsSessionValid(info), info.early_data_capable);
    } else {
        LOG_DEBUG("Session for %s can be used for 0-RTT, remaining lifetime: %u seconds", server_name.c_str(),
            GetSessionRemainingLifetime(info));
    }
...
void SessionCache::CleanupExpiredSessions() {
    ...
    if (before_size != after_size) {
        LOG_INFO("Cleaned up %zu expired sessions, cache size: %zu", before_size - after_size, after_size);
    }
}
...
void SessionCache::EvictLRUEntries() {
    while (sessions_cache_.size() > max_cache_size_) {
        ...
        LOG_DEBUG("Evicting LRU session for %s (cache size: %zu, max: %u)", lru_server_name.c_str(),
            sessions_cache_.size(), max_cache_size_);
