...
public:
    ...
    // check remote bidirection stream limit
    void SetRemoteStreamID(uint64_t id);
    bool CheckRemoteBidirectionStreamLimit(uint64_t stream_id, std::shared_ptr<IFrame> send_frame);
    bool CheckRemoteUnidirectionStreamLimit(uint64_t stream_id, std::shared_ptr<IFrame> send_frame);
...
private:
    ...
    uint64_t _remote_cur_max_stream_id;
