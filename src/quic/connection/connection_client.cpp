...
bool ClientConnection::DialSetupTLS(std::shared_ptr<TLSClientConnection> tls_conn, const common::Address& addr,
    ...
    // preference is preserved in |preferred_version| (and surfaced to the
...
bool ClientConnection::HandleHandshakeDoneFrame(std::shared_ptr<IFrame> /*frame*/) {
    ...
    if (handshake_done_processed_) {
        LOG_DEBUG("HANDSHAKE_DONE already processed, ignoring duplicate frame");
        return true;
    }
...
bool ClientConnection::OnRetryPacket(const std::shared_ptr<IPacket>& packet) {
    ...
    if (header_src.Valid() && token_span.Valid()) {
        // RFC 9001 §5.8: the integrity tag is computed over the pseudo-packet
        //   ODCID Length (8) || ODCID || Retry Packet (without the tag)
        // The Retry Packet on the wire is header || token (no ODCID field).
        // VerifyRetryIntegrityTag prepends ODCID Length || ODCID itself, so the
        // verification body here must be exactly header || token.
        ...
        uint8_t* p = retry_without_tag.data();
        std::memcpy(p, header_src.GetStart(), header_len);
        p += header_len;
        std::memcpy(p, token_span.GetStart(), token_len);
    ...
    if (crypto_stream) {
        OnStreamDataReady(crypto_stream);
    } else {
