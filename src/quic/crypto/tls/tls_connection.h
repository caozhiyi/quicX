...
public:
    ...
    virtual void SetReadSecret(
        SSL* ssl, EncryptionLevel level, const SSL_CIPHER* cipher, const uint8_t* secret, size_t secret_len) = 0;

    virtual void SetWriteSecret(
        SSL* ssl, EncryptionLevel level, const SSL_CIPHER* cipher, const uint8_t* secret, size_t secret_len) = 0;

    virtual void WriteMessage(EncryptionLevel level, const uint8_t* data, size_t len) = 0;
...
public:
    ...
    // add crypto data
    virtual bool ProcessCryptoData(uint8_t* data, uint32_t len, uint16_t encryption_level);
...
public:
    static int32_t SetReadSecret(
        SSL* ssl, ssl_encryption_level_t level, const SSL_CIPHER* cipher, const uint8_t* secret, size_t secret_len);
    static int32_t SetWriteSecret(
        SSL* ssl, ssl_encryption_level_t level, const SSL_CIPHER* cipher, const uint8_t* secret, size_t secret_len);
    static int32_t AddHandshakeData(SSL* ssl, ssl_encryption_level_t level, const uint8_t* data, size_t len);
