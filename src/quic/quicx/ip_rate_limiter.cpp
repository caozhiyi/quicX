...
IPRateLimiter::IPRateLimiter(uint32_t max_cache_size, uint32_t rate_threshold, uint32_t window_seconds):
    max_cache_size_(max_cache_size),
    rate_threshold_(rate_threshold),
    window_duration_(window_seconds) {
    LOG_DEBUG("IPRateLimiter: initialized with cache_size=%u, threshold=%u, window=%us", max_cache_size, rate_threshold,
        window_seconds);
}
...
void IPRateLimiter::RecordConnection(const std::string& ip) {
    ...
    if (entry.count == rate_threshold_) {
        LOG_WARN("IPRateLimiter: IP %s reached threshold (%u connections in window)", ip.c_str(), rate_threshold_);
    }
}
...
void IPRateLimiter::UpdateConfig(uint32_t rate_threshold, uint32_t window_seconds) {
    ...
    LOG_DEBUG("IPRateLimiter: config updated threshold=%u, window=%us", rate_threshold, window_seconds);
}
