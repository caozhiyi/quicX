...
bool FrameDecode::DecodeFrame(std::shared_ptr<Buffer> buffer, std::shared_ptr<AlloterWrap> alloter, std::vector<std::shared_ptr<Frame>>& frames) {
    if(!buffer || !alloter) {
        return false;
    }
...
bool DecodeFrame(std::shared_ptr<Buffer> buffer, std::shared_ptr<AlloterWrap> alloter, std::vector<std::shared_ptr<Frame>>& frames) {
    return FrameDecode::Instance().DecodeFrame(buffer, alloter, frames);
}
