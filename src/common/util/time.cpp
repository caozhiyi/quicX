    ...
    switch (unit) {
        case FormatTimeUnit::kMillisecondFormat:
            len = snprintf(buf, len, "%04d-%02d-%02d:%02d:%02d:%02d:%03d", 1900 + time.tm_year, 1 + time.tm_mon,
                time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec, millisecond);
            break;
        case FormatTimeUnit::kSecondFormat:
            len = snprintf(buf, len, "%04d-%02d-%02d:%02d:%02d:%02d", 1900 + time.tm_year, 1 + time.tm_mon,
                time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec);
            break;
        case FormatTimeUnit::kMinuteFormat:
            len = snprintf(buf, len, "%04d-%02d-%02d:%02d:%02d", 1900 + time.tm_year, 1 + time.tm_mon, time.tm_mday,
                time.tm_hour, time.tm_min);
            break;
        case FormatTimeUnit::kHourFormat:
            len = snprintf(
                buf, len, "%04d-%02d-%02d:%02d", 1900 + time.tm_year, 1 + time.tm_mon, time.tm_mday, time.tm_hour);
            break;
