#ifndef COMMON_ALLOCATOR_IF_ALLOCATOR
#define COMMON_ALLOCATOR_IF_ALLOCATOR
 ...
 *
 * Holds only a raw IAllocator* (one pointer, no atomic refcount). Intended to be
 * used as the Deleter for std::unique_ptr, producing a zero-overhead RAII
 * wrapper around pool-allocated objects.
 *
 * Lifetime contract: the referenced IAllocator MUST outlive every
 * PoolUniquePtr<T> it produced. In the "one PoolAllocator per Connection"
 * model this is naturally true because the allocator is owned by the
...
public:
    PoolDeleter() noexcept:
        allocator_(nullptr) {}
    explicit PoolDeleter(IAllocator* a) noexcept:
        allocator_(a) {}
...
/**
 * @brief std::allocator-compatible adapter over IAllocator.
 *
 * Plug into std::allocate_shared<T>(PoolStdAllocator<T>(allocator), args...)
 ...
 * from.
 *
 * Lifetime contract: the referenced IAllocator MUST outlive the resulting
 * shared_ptr AND any weak_ptr observing it. In the "one PoolAllocator per
 * Connection" model this is naturally true because the allocator is owned by
 ...
 * metadata. If that total exceeds PoolAllocator::kDefaultMaxBytes (256B), the
 * allocation transparently falls through to the backing NormalAllocator
 * (plain malloc); behaviour stays correct, but the pool benefit is lost
 * for that object.
 *
 * Thread-safety: inherits the underlying IAllocator's contract. PoolAllocator
...
public:
    ...
    PoolStdAllocator() noexcept:
        allocator_(nullptr) {}
    explicit PoolStdAllocator(IAllocator* a) noexcept:
        allocator_(a) {}
    ...
    T* allocate(std::size_t n) {
        // PoolAllocator::Free needs the size, and the STL contract guarantees the
        // matching deallocate(p, n) is called with the same n. So we don't have
        // to remember the size out-of-band.
        const std::size_t bytes = n * sizeof(T);
        void* p = allocator_->MallocAlign(static_cast<uint32_t>(bytes));
        if (!p) {
    ...
    void deallocate(T* p, std::size_t n) noexcept {
        if (!p) return;
        void* data = static_cast<void*>(p);
        allocator_->Free(data, static_cast<uint32_t>(n * sizeof(T)));
    }
    ...
    bool operator==(const PoolStdAllocator<U>& o) const noexcept {
        return allocator_ == o.GetAllocator();
    }
    template <typename U>
    bool operator!=(const PoolStdAllocator<U>& o) const noexcept {
        return allocator_ != o.GetAllocator();
...
class allocatorWrap {
public:
    allocatorWrap(std::shared_ptr<IAllocator> a):
        allocator_(a) {}
    ~allocatorWrap() {}
     ...
     *
     * Zero-overhead RAII: the deleter is a single raw IAllocator* (no control
     ...
     *   - the deleter capture of std::shared_ptr<IAllocator> and its atomic
     ...
     * PoolAllocator::kDefaultMaxBytes (256B) to stay in the free-list path;
     * larger combined sizes transparently fall through to the backing
     * NormalAllocator (correct, but not pooled).
     *
     * @tparam T Object type
     * @tparam Args Constructor argument types
     * @param args Constructor arguments
     ...
     *       (NOT the pool);
     *   (2) the deleter captures std::shared_ptr<IAllocator>, adding atomic
     ...
     * deleters. The pointer stays valid as long as this allocatorWrap is
     * alive.
     */
    IAllocator* GetAllocator() const noexcept { return allocator_.get(); }
...
template <typename T, typename... Args>
T* allocatorWrap::PoolNew(Args&&... args) {
    uint32_t sz = sizeof(T);

    void* data = allocator_->MallocAlign(sz);
    if (!data) {
        return nullptr;
    }
...
template <typename T, typename... Args>
PoolUniquePtr<T> allocatorWrap::PoolMakeUnique(Args&&... args) {
    T* p = PoolNew<T>(std::forward<Args>(args)...);
    // Stateless deleter: only a raw IAllocator*. No atomic refcount, no capture.
    return PoolUniquePtr<T>(p, PoolDeleter<T>(allocator_.get()));
}

template <typename T, typename... Args>
std::shared_ptr<T> allocatorWrap::PoolMakeShared(Args&&... args) {
    ...
    return std::allocate_shared<T>(PoolStdAllocator<T>(allocator_.get()), std::forward<Args>(args)...);
}

template <typename T, typename... Args>
std::shared_ptr<T> allocatorWrap::PoolNewSharePtr(Args&&... args) {
...
template <typename T>
void allocatorWrap::PoolDelete(T* c) {
    if (!c) {
        return;
    }
    ...
    void* data = (void*)c;
    allocator_->Free(data, len);
}

template <typename T>
T* allocatorWrap::PoolMalloc(uint32_t sz) {
    return (T*)allocator_->MallocAlign(sz);
}

template <typename T>
std::shared_ptr<T> allocatorWrap::PoolMallocSharePtr(uint32_t size) {
    T* ret = PoolMalloc<T>(size);
    // Capture only the raw IAllocator*: no atomic refcount on the captured
    // shared_ptr<IAllocator>, smaller deleter footprint. Lifetime contract is
    // the same as PoolUniquePtr (allocator must outlive the shared_ptr).
    IAllocator* raw = allocator_.get();
...
template <typename T>
void allocatorWrap::PoolFree(T* m, uint32_t len) {
    if (!m) {
        return;
    }
    void* data = (void*)m;
