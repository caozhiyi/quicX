...
bool KqueueEventDriver::AddFd(int32_t sockfd, int32_t events) {
    ...
    if (nchanges > 0) {
        if (kevent(kqueue_fd_, changelist, nchanges, nullptr, 0, nullptr) < 0) {
            LOG_ERROR("Failed to add events for fd %d to kqueue: %s", sockfd, strerror(errno));
            return false;
        }
    }

    LOG_DEBUG("Added fd %d to kqueue monitoring", sockfd);
    return true;
}
...
bool KqueueEventDriver::RemoveFd(int32_t sockfd) {
    ...
    LOG_DEBUG("Removed fd %d from kqueue monitoring", sockfd);
    return true;
}
...
int KqueueEventDriver::Wait(std::vector<Event>& events, int timeout_ms) {
    ...
    // per-loop allocation + default-construction of max_events_ kevent
    // structures, which showed up prominently in profiling.
    if (kqueue_events_scratch_.size() < static_cast<size_t>(max_events_)) {
        kqueue_events_scratch_.resize(max_events_);
    }
    ...
    if (nfds > 0) {
        ...
        for (int i = 0; i < nfds; ++i) {
            ...
            if (kqueue_events_scratch_[i].ident == static_cast<uintptr_t>(wakeup_fd_[0])) {
                // This is a wakeup event, consume the data
                char buffer[64];
                ssize_t bytes_read = read(wakeup_fd_[0], buffer, sizeof(buffer));
                LOG_DEBUG("KqueueEventDriver::Wait: read %zd bytes from wakeup pipe", bytes_read);
                if (bytes_read < 0) {
