...
struct MetricsStd {
    ...
    // BuildDataPacket phase decomposition (Histogram, μs).
    // Together these break the DiagBuildLatencyUs envelope into:
    //   frames  : visitor ctor + control frames + stream frames + payload-padding
    //   setup   : packet object alloc + header/CID/version/PN assignment
    //   encode  : packet->Encode (AEAD seal + header protection)
    //   record  : visitor.GetStreamDataInfo + send_control.OnPacketSend + result fill
    // Use these to pinpoint which phase is dominating before optimizing.
    static MetricID DiagBuildPhaseFramesUs;
    static MetricID DiagBuildPhaseSetupUs;
    static MetricID DiagBuildPhaseEncodeUs;
    static MetricID DiagBuildPhaseRecordUs;
    ...
    static MetricID DiagTrySendBurstPkts;  // Packets per TrySendBurst() call
