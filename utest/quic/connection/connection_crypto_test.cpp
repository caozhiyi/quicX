...
bool Decrypt(std::shared_ptr<ICryptographer>& cryptographer, std::shared_ptr<IPacket> packet,
    ...
    LOG_DEBUG("decrypt packet start:%p", packet->GetSrcBuffer().GetStart());
    LOG_DEBUG("decrypt packet num len:%d", packet->GetPacketNumOffset() + header->GetPacketNumberLength());
    // decrypto packet
...
bool Encrypt(std::shared_ptr<ICryptographer>& cryptographer, std::shared_ptr<IPacket> packet, 
    ...
    LOG_DEBUG("encrypt packet start:%p", packet->GetSrcBuffer().GetStart());
    LOG_DEBUG("encrypt packet num len:%d", packet->GetPacketNumOffset() + header->GetPacketNumberLength());
    ...
    LOG_DEBUG("encrypt payload start:%p", payload.GetStart());
    LOG_DEBUG("encrypt payload end:%p", payload.GetEnd());
...
TEST(connnection_crypto_utest, generator) {
    ...
    packet1->GetHeader()->SetPacketNumberLength(2);
    ...
    // decrypt
    EXPECT_TRUE(Decrypt(ser_cryptographer, packet2, cipher_buffer));
    EXPECT_FALSE(packet2->DecodeAfterDecrypt(cipher_buffer));
