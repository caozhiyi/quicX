...
TEST_F(StaticTableTest, LookupByIndex) {
    ...
    EXPECT_EQ(item->_name, ":authority");
    EXPECT_EQ(item->_value, "");

    item = table_->FindHeaderItem(15);
    EXPECT_TRUE(item != nullptr);
    EXPECT_EQ(item->_name, ":method");
    EXPECT_EQ(item->_value, "CONNECT");
}
...
TEST_F(StaticTableTest, LookupByNameAndValue) {
    ...
    EXPECT_TRUE(item != nullptr);
    EXPECT_EQ(item->_name, ":method");
    EXPECT_EQ(item->_value, "GET");

    item = table_->FindHeaderItem(31);
    EXPECT_TRUE(item != nullptr);
    EXPECT_EQ(item->_name, "accept-encoding");
    EXPECT_EQ(item->_value, "gzip, deflate, br");
